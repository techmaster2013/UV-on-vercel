...
const normalizeUrl = (value) => {
    let url = value.trim();
    if (!url.includes('.') || url.includes(' ')) url = 'https://www.bing.com/search?q=' + encodeURIComponent(url);
...
const addBookmark = (url) => { if (!url) return; bookmarks = bookmarks.includes(url) ? bookmarks.filter(x => x !== url) : [url, ...bookmarks].slice(0, 30); write('pp_bookmarks', bookmarks); renderSaved(); };

if ('serviceWorker' in navigator) {
    window.addEventListener('load', () => {
        if (window.__uv$config) navigator.serviceWorker.register('/uv/sw.js', { scope: __uv$config.prefix }).catch(() => showError('Proxy service worker could not be registered.'));
    });
}
...
form?.addEventListener('submit', (e) => {
    ...
    showError('');
    const now = Date.now();
    if (now - lastSubmit < 3000) { showError('Please wait a moment before sending another request.'); return; }
    lastSubmit = now;
    const value = input.value.trim();
    if (!value) { showError('Enter a URL or search query first.'); return; }
    const url = normalizeUrl(value);
    addHistory(url);
    if (!window.__uv$config) { showError('Proxy configuration is not loaded yet.'); return; }
